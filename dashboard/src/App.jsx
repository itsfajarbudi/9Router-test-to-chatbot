...
const ChatbotView = () => {
  ...
  const handleSend = (e) => {
    ...
    // Simulate network delay and 9Router logic
    setTimeout(() => {
      // Pick a random model for simulation
      const randomAI = AI_MODELS[Math.floor(Math.random() * AI_MODELS.length)];
      
      const aiMsg = { 
        id: Date.now() + 1, 
        role: 'ai', 
        text: `This is a simulated response generated by ${randomAI.name}. Your request was routed based on current latency and cost metrics.`, 
        model: randomAI 
      };
